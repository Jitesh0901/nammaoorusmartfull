...
async function updateImports() {
  ...
  const importRegex =
    /import\s+(\w+)\s+from\s+'([^']+\/([\w\d]+)\.(webp|jpeg|jpg|png))';/g;

  content = content.replace(
    importRegex,
    (match, varName, path, filename, ext) => {
      // Skip if already a thumbnail
      if (
        path.includes("_thumbnail") ||
        path.includes("_medium") ||
        path.includes("_large")
      ) {
        return match;
      }
      ...
      const newPath = path.replace(
        new RegExp(`${filename}\\.${ext}$`),
        `${filename}_thumbnail.webp`,
      );
      console.log(
        `  ✓ ${varName}: ${filename}.${ext} → ${filename}_thumbnail.webp`,
      );
