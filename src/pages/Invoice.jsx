...
export default function Invoice() {
  ...
  const handleWhatsApp = () => {
    ...
    invoice.items.forEach((it) => {
      ...
      lines.push(
        `  Qty: ${it.qty}, Price: ₹${formatCurrency(it.price)}, Total: ₹${formatCurrency(it.price * it.qty)}`,
      );
    });
      ...
      <div className="max-w-4xl mx-auto bg-white rounded-lg shadow-lg p-8">
        <div className="flex gap-3 mb-8">
          <button
            ...
            className="flex items-center gap-2 bg-green-600 text-white px-6 py-3 rounded-lg font-bold hover:bg-green-500"
          >
            <Share2 size={20} /> WhatsApp
          </button>
          <button
            onClick={() => navigate("/")}
            className="flex items-center gap-2 border px-6 py-3 rounded-lg font-bold hover:bg-gray-50"
          >
        ...
        <div className="border-b pb-6 mb-6 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <img
              src={LOGO}
              alt="logo"
              className="h-16 w-16 object-contain rounded-full bg-gray-100 p-2"
            />
            <div>
