...
const FeatureCard = ({ icon: Icon, title, description, delay, counterValue, counterSuffix, isLast }) => {
  return (
    <div className="relative flex flex-col items-center">
      ...
      >
        <div className="absolute inset-0 bg-gradient-to-br from-orange-50/0 to-orange-50/0 group-hover:from-orange-50/50 group-hover:to-transparent transition-all duration-500 -z-10" />
        ...
        >
          <Icon size={24} className="sm:w-8 sm:h-8 transition-transform duration-500" />
        </motion.div>
        
        <h3 className="text-lg sm:text-xl font-bold text-slate-900 mb-2 sm:mb-3 flex items-center gap-2">
          {counterValue ? (
            <div className="flex items-baseline gap-1">
               <span className="text-xl sm:text-2xl font-black text-slate-950"><Counter value={counterValue} suffix={counterSuffix} /></span>
               <span className="text-xs sm:text-sm font-bold text-slate-400 tracking-tight uppercase">{title.replace(/[0-9+]/g, '')}</span>
            </div>
          ) : (
        ...
        <p className="text-slate-500 text-sm sm:text-[15px] leading-relaxed font-medium">
          {description}
        </p>
      ...
      {/* Animated Link Chain for Mobile */}
      {!isLast && (
        <div className="md:hidden h-8 w-px relative overflow-hidden flex flex-col items-center">
            <motion.div 
                initial={{ height: 0 }}
...
export default function ServicesPage({ onScrollToProduct }) {
  const features = [
    {
      icon: Shield,
  ...
  return (
    ...
    >
      <Suspense fallback={<div className="h-screen bg-black flex items-center justify-center text-yellow-400">Loading Showcase...</div>}>
         <CompanyShowcase />
      </Suspense>
